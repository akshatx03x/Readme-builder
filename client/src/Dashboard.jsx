...
const Dashboard = () => {
  ...
  // Create a ref for the GeminiChatUI component
  ...
  const handleMakeReadme = (repo) => {
    // Check if the ref is connected and the function exists
    if (geminiChatRef.current && geminiChatRef.current.generateReadme) {
      geminiChatRef.current.generateReadme(repo);
      // Smooth scroll to the chat UI
      const chatElement = document.getElementById('gemini-chat-ui');
      if (chatElement) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 to-zinc-900 p-8 flex font-inter">
      <div className="w-[30%] max-w-md ml-8">
        <div className="mb-5">
          ...
          </a><span className="px-5"><button
            onClick={handleLogout}
            className="w-20 h-9 items-center justify-center gap-3 bg-gray-800/50 backdrop-blur-md border border-red-700/50 hover:bg-red-700/50 text-red-400 py-1 rounded-full font-medium transition-all duration-200 shadow-md hover:shadow-lg"
          >
            Logout
          </button></span>
      ...
      <div className="w-[70%] max-w-4xl mr-8 flex items-center justify-center">
        ...
        <GeminiChatUI ref={geminiChatRef} id="gemini-chat-ui" />
      </div>
    </div>
  );
};
